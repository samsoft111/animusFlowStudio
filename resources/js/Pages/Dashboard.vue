  ...
  <AppLayout :title="t('dashboard.title')">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      ...
      <PublishedPreviewCard
        :label="t('themes.status.published') + ' ' + t('nav.themes')"
        :count="stats.published_themes"
        :items="publishedThemes"
        list-href="/themes?status=published"
        :preview-url="(th) => `/preview/theme/${th.uuid}`"
        :preview-title="t('dashboard.preview_theme')"
        :preview-label="t('dashboard.preview')"
        :search-placeholder="t('dashboard.search_theme')"
        :no-match-text="t('dashboard.no_match')" />

      <!-- Plugins publicados — mesma lógica, preview do widget -->
      <PublishedPreviewCard
        :label="t('themes.status.published') + ' ' + t('nav.plugins')"
        :count="stats.published_plugins"
        :items="publishedPlugins"
        list-href="/plugins?status=published"
        :preview-url="(pl) => `/plugins/${pl.uuid}/preview-widget`"
        :preview-title="t('dashboard.preview_plugin')"
        :preview-label="t('dashboard.preview')"
        :search-placeholder="t('dashboard.search_plugin')"
        :no-match-text="t('dashboard.no_plugin_match')" />
    </div>
...
import PublishedPreviewCard from '@/Components/PublishedPreviewCard.vue';
...
defineProps({
  ...
  publishedPlugins: { type: Array,  default: () => [] },
});
